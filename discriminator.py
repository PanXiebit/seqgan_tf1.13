...
class Discriminator(tf.keras.layers.Layer):
    ...
    def __init__(self, seq_len, vocab_size, embed_size, filter_sizes, num_filters, num_classes,
                 l2_reg_lambda=0.0, dropout_keep_prob=0.1, d_lr_rate=1e-4):
        ...
        #self.d_optimizer = tf.keras.optimizers.Adam(d_lr_rate)
    ...
    def call(self, input_x, input_y):
        ...
        losses = tf.reduce_mean(self.loss_obj(y_pred=self.ypred_for_auc, y_true=input_y))  # [batch] -> 1
        tf.identity(losses, name="cross_entropy")
        # tf.summary.scalar("cross_entropy", losses, step=)
